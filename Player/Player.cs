...
namespace MCForge
{
    public sealed partial class Player : IDisposable
    {
        ...
        public System.Timers.Timer cookietimer = new System.Timers.Timer(10000);
        ...
        public int blockCount = Server.blocklimithuman;
        ...
        public bool womwarning = false;
        public string invitedtoteam = "";
        public string teamname = "";
        public string teamcolor = "";
        public bool reportedforglitching = false;
        public bool invisible = false;
        public bool recentlywarned = false;
        public string preprefix = "";
        public int winstreakcount = 0;
        public string recentlevelvote = "";
        ...
        public DateTime infecttime = new DateTime();
        public int maximuminfected = 0;
        public int maximumsurvived = 0;
        public bool ignoreirc = false;
        public bool stuffed = false;
        // -------------- Awards --------------------
        public List<string> achievements = new List<string>();
        public string achievementnumbers = "";
        public string infectedfrom = "";
        ...
        public void save()
        {
            ...
            string commandString =
                ...
                ", maximumsurvived=" + maximumsurvived +
                ", playersinfected=" + playersinfected +
                ", maximuminfected=" + maximuminfected +
                ", totalBlocks=" + overallBlocks + " + " + loginBlocks +
                ", totalKicked=" + totalKicked +
                ", TimeSpent='" + time +
                "', achievements='" + achievementnumbers.ToString() +
                "' WHERE Name='" + name + "'";
            if (MySQLSave != null)
        ...
        static void Receive(IAsyncResult result)
        {
            ...
            try
            {
                int length = p.socket.EndReceive(result); //Länge der Zeichen de reinkommt
            ...
            {
                // Player is no longer connect ed, socket was closed
        ...
        void HandleLogin(byte[] message)
        {
            try
            {
                ...
                try
                {
                    ...
                    Server.TempBan tBan2 = Server.tempBans.Find(tB => tB.ip.ToLower() == ip.ToLower());
                    if (tBan.allowedJoin < DateTime.Now && tBan2.allowedJoin < DateTime.Now)
                    {
                        Server.tempBans.Remove(tBan);
                    }
                    else
                    {
                        Kick("You're still temporary banned!");
                    }
                }
                catch { }
                ...
                if (Server.bannedIP.Contains(ip))
                {
                    if (Server.useWhitelist)
                    {
                        if (!onWhitelist)
                        {
                            Kick("You are ipbanned! appeal on www.minemaniacs.freeforums.org");
                            return;
                        }
                    }
                    else
                ...
                if (Server.zombieipbans.Contains(ip))
                {
                    Kick("You are ipbanned from all zombie servers");
                }
                if (Server.zombiebans.Contains(name))
                {
                    Kick("You are banned from all zombie servers");
                }
                ...
                {
                    if (Ban.banonbuild(name)) Kick("You are banned! appeal on www.minemaniacs.freeforums.org");
                    if (Ban.ipbanonbuild(ip)) Kick("You are ipbanned! appeal on www.minemaniacs.freeforums.org");
                }
                if (!Server.devs.Contains(name.ToLower()) && !VIP.Find(this))
                {
                    // Check to see how many guests we have
                    if (Player.players.Count >= Server.players && !IPInPrivateRange(ip))
                    {
                        Server.s.Log(this.name + " couldn't log in - server full!");
                    ...
                    {
                        // Check to see how many guests we have
                        int currentNumOfGuests = Player.players.Count(pl => pl.group.Permission < LevelPermission.Builder);
            ...
            if (playerDb.Rows.Count == 0)
            {
                ...
                this.playersinfected = 0;
                this.maximuminfected = 0;
                this.maximumsurvived = 0;
                ...
                Player.GlobalMessage("Welcome " + name + "! This is their first visit!");
                SendMessage("Make sure you read the /rules and /faq");
            ...
            else
            {
                ...
                this.teamname = Player.GetTeamName(this);
                this.teamcolor = c.Parse(Player.GetTeamColor(this));
                SetPrefix();
                ...
                roundssurvived = int.Parse(playerDb.Rows[0]["roundssurvived"].ToString());
                maximumsurvived = int.Parse(playerDb.Rows[0]["maximumsurvived"].ToString());
                playersinfected = int.Parse(playerDb.Rows[0]["playersinfected"].ToString());
                maximuminfected = int.Parse(playerDb.Rows[0]["maximuminfected"].ToString());
                totalKicked = int.Parse(playerDb.Rows[0]["totalkicked"].ToString());
                GetAchievements(playerDb.Rows[0]["achievements"].ToString());
                SendMessage("Greetings " + color + prefix + name + Server.DefaultColor + "! this is your " + (totalLogins +1) + " visit!");
            ...
            if (this.group.Permission < Server.adminchatperm || Server.adminsjoinsilent == false)
            {
                ...
                {
                    if (File.Exists("text/login/" + this.name + ".txt")) GlobalChat(this, "&a+ " + this.color + this.prefix + this.name + Server.DefaultColor + " " + File.ReadAllText("text/login/" + this.name + ".txt"), false);
                    else GlobalChat(this, "&a+ " + this.color + this.prefix + this.name + Server.DefaultColor + " joined the server", false);
                }
                if (this.group.Permission > LevelPermission.Guest)
                {
                    if (File.Exists("text/login/" + this.name + ".txt")) GlobalChat(this, "&a+ " + this.color + this.prefix + this.name + Server.DefaultColor + " " + File.ReadAllText("text/login/" + this.name + ".txt"), false);
            ...
            if (Convert.ToInt32(this.group.Permission) >= 10)
            {
                this.voice = true;
                this.voicestring = "&f+";
            }
            try
            {
                ...
                {
                    this.SendMessage("%c-------------------------------------------");
                    this.SendMessage("%cYou are a zombie");
                    this.SendMessage("%cBecause you joined in the middle of a round");
                    this.SendMessage("%c-------------------------------------------");
                    Server.zombie.InfectPlayer(this);
                }
            }
            catch (Exception x)
            {
        ...
        {
            if (winstreakcount <= 0) preprefix = "";
            else if (winstreakcount == 1) preprefix = "%4*";
            else if (winstreakcount == 2) preprefix = "%7*"; 
            else if (winstreakcount == 3) preprefix = "%6*";
            else preprefix = "%6"+winstreakcount.ToString();

            if (title == "" && teamname == "") prefix = preprefix + group.color;
            else if (title == "")
                prefix = preprefix + group.color + "<" + teamcolor + teamname + group.color + ">";
            else if (teamname == "") 
                prefix = preprefix + group.color +  "[" + titlecolor + title + group.color +  "] ";
            else 
                prefix= preprefix + group.color + "<" + teamcolor + teamname + group.color + ">" + "[" + titlecolor + title + group.color + "] ";

            /*else if (teamname == "" && teamcolor == "") prefix = preprefix + group.color + "[" + titlecolor + title + group.color + "] ";
            else if (teamname == "" && teamcolor == "") prefix = preprefix + group.color + "[" + title + group.color + "] ";
            else if (teamname != "" && teamcolor == "") prefix = preprefix + group.color + "<" + teamname + ">" + "[" + title + group.color + "] ";
            else if (teamname != "" && teamcolor != "") prefix 
            //prefix = (title == "") ? "" : (titlecolor == "") ? "[" + title + "] " : "[" + titlecolor + title + color + "] ";*/
        }
        public void OffenseNote(DateTime when, string what, string reason, string reporter)
        {
            if (!Directory.Exists("text/offensenotes")) Directory.CreateDirectory("text/offensenotes");
            string path = "text/offensenotes/"+this.name+".txt";
            if (!File.Exists(path)) File.Create(path).Dispose();
            try
            {
                StreamWriter sw = File.AppendText(path);
                sw.WriteLine(when.Day+"."+when.Month+"."+when.Year+": "+ what + " : " + reason + " by " + reporter );
                sw.Close();
        ...
        public void GetAchievements(string IDS)
        {
            if (IDS == "") return;
            string[] numbers = Regex.Split(IDS, ",");
            foreach (string theid in numbers)
            {
                if (theid != "")
                {
                    this.achievements.Add(GetAchievementName(theid));
                    this.achievementnumbers += ("," + theid);
                }
            }
        }
        public string GetAchievementName(string number)
        {
            if (number == "") return "";
            string output = "";
            try
            {
                DataTable achievement = Database.fillData("SELECT * FROM Achievements WHERE ID=" + Convert.ToInt32(number) + ";");
                output = achievement.Rows[0]["name"].ToString();
                achievement.Dispose();
            }
            catch
            {
                Server.s.Log("ERROR: #025");
            }
            return output;
        }
        public string GetAchievementNumber(string name)
        {
            if (name == "") return "";
            string output = "";
            try
            {
                DataTable achievement = Database.fillData("SELECT * FROM Achievements WHERE name='" + name + "';");
                output = achievement.Rows[0]["ID"].ToString();
                achievement.Dispose();
            }
            catch
            {
                Server.s.Log("ERROR: #024");
            }
            return output;
        }
        public void Achieve(string name)
        {
            if (!this.achievements.Contains(name))
            {
                ...
                this.achievementnumbers += "," + GetAchievementNumber(name);
                Player.SendMessage(this, c.aqua + "You just gained the achievment: " + name);
            }
        }
        void HandleBlockchange(byte[] message)
        {
            ...
            try
            {
                ...
                int playerx = this.pos[0]/32;
                int playery = this.pos[1]/32;
                int playerz = this.pos[2]/32;
                byte action = message[6];
                byte type = message[7];

                if (action == 1 && Server.noPillaring && !Block.Walkthrough(type))
                {
                    if (!referee)
                    {
                        if (lastYblock == y - 1 && lastXblock == x && lastZblock == z && z == playerz && x == playerx)
                        {
                            blocksStacked++;
                        }
                        else
                        ...
                        if (blocksStacked == 3)
                        {
                            SendMessage("You are pillaring! Last warning before kick");
                        }
                        if (blocksStacked == 4)
                        {
                            this.Kick("No pillaring allowed");
                            this.OffenseNote(DateTime.Now, "%5kicked%c", "autopillardetection", "system");
                        }
                    }
                }
        ...
        public void manualChange(ushort x, ushort y, ushort z, byte action, byte type)
        {
            ...
            bool isspleefing = false;

            if (infected)
            {
                foreach (Player p in ZombieGame.infectd)
                {
                    ushort[] conpos = new ushort[3] { (ushort)((p.pos[0]-16) / 32), 
                    (ushort)(((p.pos[1]-32) / 32)-1), (ushort)((p.pos[2]-16) / 32) };
                    if (conpos[0] == x && conpos[1] == y && conpos[2] == z && p != this) isspleefing = true;
                }
            }
            else
            {
                foreach (Player p in ZombieGame.alive)
                {
                    ushort[] conpos = new ushort[3] { (ushort)((p.pos[0]-16) / 32), 
                    (ushort)(((p.pos[1]-32) / 32)-1), (ushort)((p.pos[2]-16) / 32) };
                    if (conpos[0] == x && conpos[1] == y && conpos[2] == z && p != this) isspleefing = true;
            ...
            if (isspleefing && !referee && b != Block.air)
            { SendBlockchange(x, y, z, b); SendMessage("%cDo not spleef your own team"); return; }
            ...
            /*if (!this.referee && level.GetTile(x,y,z) == Block.blackrock)
            {
                SendBlockchange(x, y, z, level.GetTile(x, y, z));
                Player.SendMessage(this, "Cannot place this block if you aren't referee");
                return;
            }*/
            if (!Block.canPlace(this, b) && !Block.BuildIn(b) && !Block.AllowBreak(b) && (!this.referee && level.GetTile(x, y, z) == Block.blackrock))
            {
                SendMessage(c.red + "Cannot build here!");
                SendBlockchange(x, y, z, b);
                return;
            }
...
return;
            ...
            if (!this.referee && Server.noRespawn && Server.ZombieModeOn)
            {
                if (this.pos[0] >= x + 70 || this.pos[0] <= x - 70)
                {
                    ...
                    if (CheckIfInsideBlock(this) && !this.Loading && !this.reportedforglitching)
                    {
                        this.SendMessage("%cDO NOT BLOCK GLITCH");
                        Server.s.Log(this.name + " might be block glitching");
                        GlobalMessageOps(this.color + this.name + Server.DefaultColor + " might be %cblock glitching");
                        if (!this.reportedforglitching)
                        {
                            System.Timers.Timer blockglitchtimer = new System.Timers.Timer(5000);
                            this.reportedforglitching = true;
                            blockglitchtimer.Start();
                            blockglitchtimer.Elapsed += delegate { this.reportedforglitching = false; blockglitchtimer.Stop(); };
                        }
                    }
                    return;
                }
                if (this.pos[1] >= y + 210 || this.pos[1] <= y - 210)
                {
                    unchecked { SendPos((byte)-1, pos[0], pos[1], pos[2], rot[0], rot[1]); }
                    if (CheckIfInsideBlock(this) && !this.Loading && !this.reportedforglitching)
                    {
        ...
        public void HandleDeath(byte b, string customMessage = "", bool explode = false)
        {
            ...
            if (lastDeath.AddSeconds(2) < DateTime.Now)
            {
                ...
                if (level.Killer && !invincible)
                {
                    ...
                    ushort qwex = (ushort)((0.5 + this.level.spawnx) * 32);
                    ushort qwey = (ushort)((1 + this.level.spawny) * 32);
                    ushort qwez = (ushort)((0.5 + this.level.spawnz) * 32);
                    unchecked
                    {
                        this.SendPos((byte)-1, qwex, qwey, qwez,
                                    this.level.rotx,
                                    this.level.roty);
                    }
                        overallDeath++;
...
try { SendBlockchange(pos1.x, pos1.y, pos1.z, Block.waterstill); } catch { }
        ...
        void HandleChat(byte[] message)
        {
            try
            {
                ...
                if (text != "/afk")
                {
                    if (Server.afkset.Contains(this.name))
                    {
                        ...
                        this.autoafk = false;
                        if(!this.hidden)Player.GlobalMessage("-" + this.color + this.name + Server.DefaultColor + "- is no longer AFK");
                        //IRCBot.Say(this.name + " is no longer AFK");
                    }
                }
                if (text == "." || text == "t")
                {
                    Command me = Command.all.Find("me");
                    me.Use(this, "");
                    return;
                }
            ...
            hello:
                if (CapsDetection(text)) //Caps: only allow commands
                {
                    this.SendMessage("%cToo much caps your message was changed to lower!");
                    text = text.ToLower();
                }
                // People who are muted can't speak or vote
                if (muted) { this.SendMessage(c.red + "You are muted."); return; } //Muted: Only allow commands
                ...
                if (text[0] == '#' || opchat)
                {
                    ...
                    GlobalMessageOps("To Ops &f-" + group.color + name + "&f- " + newtext);
                    if (group.Permission < Server.opchatperm && !Server.devs.Contains(name.ToLower()))
                        SendMessage("To Ops &f-" + group.color + name + "&f- " + newtext);
                ...
                if (text[0] == '+' || adminchat)
                {
                    ...
                    GlobalMessageAdmins("To Admins &f-" + group.color + name + "&f- " + newtext); //to make it easy on remote
                    if (group.Permission < Server.adminchatperm && !Server.devs.Contains(name.ToLower()))
                        SendMessage("To Admins &f-" + group.color + name + "&f- " + newtext);
                ...
                if (text[0] == '-')
                {
                    ...
                    if (!Server.zombieRound){ this.SendMessage("%cNo game in progress yet"); return;}
                    foreach (Player others in Player.players)
                    {
                        if (this.referee)
                        {
                            others.SendMessage(this.group.color + this.name + c.lime + " to humans: &f" + newtext);
                            others.SendMessage(this.group.color + this.name + c.red + " to zombies: &f" + newtext);
                        }
                        else if (this.infected && (others.infected || others.referee))
                            others.SendMessage(this.group.color + this.name + c.red + " to zombies: &f" + newtext);
                        else if (!this.infected && (!others.infected || others.referee))
                            others.SendMessage(this.group.color + this.name + c.lime + " to humans: &f" + newtext);

                    }
                    if (this.referee) Server.s.Log(this.name + " to all: " + newtext);
                    else if (this.infected) Server.s.Log(this.name + " to zombies: &f" + newtext);
                    else Server.s.Log(this.name + " to humans: &f" + newtext);
                    return;
                }
                /*
                if (this.joker)
...
Disconnect();
        ...
        {
            if (this == null) { Server.s.Log(message); return; }
            if (message.Contains("[IRC]") && this.ignoreirc)
                return;
            ...
            //dont need all those variables - just wasting a short time to do all the replacement while chatting.
            if (Server.dollardollardollar)
                sb.Replace("$name", "$" + name);
            else
                sb.Replace("$nameofperson", name);
            sb.Replace("$date", DateTime.Now.ToString("yyyy-MM-dd"));
            sb.Replace("$time", DateTime.Now.ToString("HH:mm:ss"));
            sb.Replace("$ipofperson", ip);
            if (colorParse) sb.Replace("$color", color);
            sb.Replace("$rankofperson", group.name);
            //sb.Replace("$level", level.name);
            sb.Replace("$moneyofperson", money.ToString());
            ...
            sb.Replace("$survived", roundssurvived.ToString());
            sb.Replace("$infected", playersinfected.ToString());
...
changed |= 4;*/
        ...
        public static void GlobalChat(Player from, string message, bool showname)
        {
            ...
            if (showname)
            {
                String referee = "";
                if (from.referee)
                    referee = c.blue + "["+Server.refprefix+"] ";
                else if (from.infected)
        ...
        public static bool CapsDetection(string message)
        {
            if (message.Length < 6) return false;
            char[] detect = message.ToCharArray();
            int capscount = 0;
            for (int i = 0; i < detect.Count(); i++)
            {
                if (detect[i].ToString() == "A") capscount++;
                if (detect[i].ToString() == "B") capscount++;
                if (detect[i].ToString() == "C") capscount++;
                if (detect[i].ToString() == "D") capscount++;
                if (detect[i].ToString() == "E") capscount++;
                if (detect[i].ToString() == "F") capscount++;
                if (detect[i].ToString() == "G") capscount++;
                if (detect[i].ToString() == "H") capscount++;
                if (detect[i].ToString() == "I") capscount++;
                if (detect[i].ToString() == "J") capscount++;
                if (detect[i].ToString() == "K") capscount++;
                if (detect[i].ToString() == "L") capscount++;
                if (detect[i].ToString() == "M") capscount++;
                if (detect[i].ToString() == "N") capscount++;
                if (detect[i].ToString() == "O") capscount++;
                if (detect[i].ToString() == "P") capscount++;
                if (detect[i].ToString() == "Q") capscount++;
                if (detect[i].ToString() == "R") capscount++;
                if (detect[i].ToString() == "S") capscount++;
                if (detect[i].ToString() == "T") capscount++;
                if (detect[i].ToString() == "U") capscount++;
                if (detect[i].ToString() == "V") capscount++;
                if (detect[i].ToString() == "W") capscount++;
                if (detect[i].ToString() == "X") capscount++;
                if (detect[i].ToString() == "Y") capscount++;
                if (detect[i].ToString() == "Z") capscount++;
            }
            double yesno =  (double)capscount / (double)detect.Length;
            if (yesno > 0.30) return true;
            else return false;
        }
        ...
        public void leftGame(string kickString = "", bool skip = false)
        {
            ...
            try
            {
                ...
                try { ZombieGame.alive.Remove(this); }
                catch { }
                try { ZombieGame.infectd.Remove(this); }
                catch { }
                if (Server.zombieRound)
                {
                ...
                if (Server.zombie.lottery.Contains(this))
                    Server.zombie.lottery.Remove(this);
                SendKick(kickString);
                        ...
                        if (!hidden)
                        {
                            if (Server.guestLeaveNotify == true && this.group.Permission <= LevelPermission.Guest)
                            {
                                if (File.Exists("text/logout/" + name + ".txt"))
                                    ...
                                    GlobalChat(this, "&c- " + color + prefix + name + Server.DefaultColor + " disconnected", false);
                            }
                            if (this.group.Permission > LevelPermission.Guest)
                            {
                                if (File.Exists("text/logout/" + name + ".txt"))
...
//this.commThread.Abort();
        ...
        public static string GetTeamName(Player p)
        {
            ...
            {
                using (StreamReader wacha = new StreamReader("teams/" + fi.Name))
                {
                    string[] lines = File.ReadAllLines("teams/" + fi.Name);
                    foreach (string line in lines)
                        if (line == ("(TMember)" + p.name) 
                            || (line == "(TLeader)" + p.name)
                            || (line == "(THelper)" + p.name)
                            || (line == "(TCaptain)" + p.name))
                            return fi.Name.Substring(0, 3);
                }
            }
            return "";
        }
        public static string GetTeamColor(Player p)
        {
            ...
            string[] lines = File.ReadAllLines("teams/" + p.teamname + ".txt");
            foreach (string line in lines)
            {
                if (line.Contains("(TColor)"))
                {
                    string toreturn = line.Remove(0, 8);
                    return toreturn;
                }
            }
            return "";
